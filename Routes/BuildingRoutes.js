...
router.post('/add_gate/:building/:floor/:gate', async (req, res) => {
	try {
		...
		const new_building = await Building.findOneAndUpdate(
			...
			{ $push: { 'floors.$[].gates': new_gate } } // $[] is for accessing sub array elements
		);
...
router.post('/add_gate/:building/:floor/:gate/:department', async (req, res) => {
    try {
        const building = req.params.building;
        const floor = req.params.floor;
        const gate = req.params.gate;
        ...
        if (!new_building) {
            const building_exists = await Building.findOne({ name: building });
            if (!building_exists) {
                return res.status(403).json({message: `Building ${building} does not exist`});
            }
            
            const floor_exists = await Building.findOne({ name: building, 'floors.number': floor });
            if (!floor_exists) {
                return res.status(403).json({message: `Floor ${floor} does not exist in ${building} building`});
            }
            
            const gate_exists = await Building.findOne({ name: building, 'floors.number': floor, 'floors.gates.number': gate });
            if (!gate_exists) {
                return res.status(403).json({message: `Gate ${gate} does not exist in floor ${floor}`});
            }
        }
...
router.delete(
	...
	async (req, res) => {
		try {
			...
			return res.status(200).json({ message: `successfully deleted ${department} department from gate number ${gate}` });
		} catch (err) {
			return res.status(403).json({ err: err.message });
		}
	}
